...
const Snooze = () => {
  ...
  const fetchEmails = () => {
    axios.get(`${apiUrl}/snooze/get/${userMail}`)
      .then(res => {
        setEmails(res.data);
  ...
  return (
    <div>
      <Table emails={emails} setEmails={setEmails} fetchEmails={fetchEmails} deleteMail={handleInboxDelete} archiveMail={handleInboxArchive} markAsRead={handleInboxRead} snoozeMail={handleInboxSnooze} ToastComponent={ToastComponent}/>
    </div>
  );
};
